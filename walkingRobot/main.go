...
func main() {
	//fmt.Println(robotSim([]int{7, -2, -2, 7, 5}, [][]int{{-3, 2}, {-2, 1}, {0, 1}, {-2, 4}, {-1, 0}, {-2, -3}, {0, -3}, {4, 4}, {-3, 3}, {2, 2}}))
	//fmt.Println(robotSim([]int{1, 1, 3, 4, 3}, [][]int{{-1, 5}, {-4, -4}, {-3, 3}, {3, 0}, {2, 5}, {-4, 4}, {-3, 1}, {-2, -4}, {-1, -4}, {0, -3}}))
...
func robotSim(commands []int, obstacles [][]int) int {
	...
	for _, command := range commands {
		if command == -1 {
			...
			activeIdx = (activeIdx + 3) % 4
		} else {
			for step := 0; step < command; step++ {
				newX := x + directions[activeIdx][0]
				newY := y + directions[activeIdx][1]
				key := fmt.Sprintf("%d,%d", newX, newY)
				if _, ok := obstacleMap[key]; ok {
					break
				}
				x, y = newX, newY
				bestRes = max(bestRes, x*x+y*y)
			}
		}
	}
	return bestRes
